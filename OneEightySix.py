'''
    Caller(n) = S_{2n - 1}, Called(n) = S_{2n}, if Caller(n) = Called(n) then
    there is a misdial any pair of users X and Y are friend if X called Y or
    vice-versa a user Z is a friend of a friend of X if Z is friend of Y
    Author: Cody Lewis
    Since: 2018-08-29
'''
...
def lagged_fibonacci(k, fib_nums=[]):
    ...
    if len(fib_nums) < k:
        for i in range(len(fib_nums) + 1, k + 1):
            if i >= 1 and i <= 55:
                fib_nums.append((100003 - 200003 * i + 300007 * i**3)
                                % 1000000)
            elif i >= 56:
                fib_nums.append((fib_nums[i - 24] + fib_nums[i - 55])
                                % 1000000)
    return fib_nums[k - 1]
...
class DisSet():
    ...
    def __init__(self):
        '''
            Initialize with the maximum number of nodes (determined by mod on
            LFG)
        '''
...
if __name__ == "__main__":
    ...
    print("99% or more users are connected by friends with the PM at k = {}"
          .format(find_friends()))
